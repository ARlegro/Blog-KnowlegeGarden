---
{"dg-publish":true,"permalink":"/DevStudy/DB/Fundamentals of DB engineering/Indexing/인덱싱에 대한 개념 및 각종 실험/","noteIcon":"","created":"2025-06-08T14:27:47.720+09:00","updated":"2025-12-07T20:49:19.171+09:00"}
---
...
- [[#1.  인덱스란 ❓|1.  인덱스란 ❓]]
	- [[#1.  인덱스란 ❓#1.1.  정의|1.1.  정의]]
	- [[#1.  인덱스란 ❓#1.2.  인덱스의 핵심 메커니즘|1.2.  인덱스의 핵심 메커니즘]]
- [[#성능 비교|성능 비교]]
- [[#성능 비교#1.  성능 비교하기 전 세팅|1.  성능 비교하기 전 세팅]]
	- [[#1.  성능 비교하기 전 세팅#1.1.  기본 세팅 및 분석|1.1.  기본 세팅 및 분석]]
	- [[#1.  성능 비교하기 전 세팅#1.2.  더미 데이터 - 100만|1.2.  더미 데이터 - 100만]]
	- [[#1.  성능 비교하기 전 세팅#1.3.  EXPLAIN ANALYZE 도입|1.3.  EXPLAIN ANALYZE 도입]]
- [[#성능 비교#2.  본격 실험|2.  본격 실험]]
	- [[#2.  본격 실험#2.1.  시나리오 1. Index Only Scan|2.1.  시나리오 1. Index Only Scan]]
	- [[#2.  본격 실험#2.2.  시나리오 2. name 조회하기|2.2.  시나리오 2. name 조회하기]]
		- [[#2.2.  시나리오 2. name 조회하기#2.2.1.  실행|2.2.1.  실행]]
		- [[#2.2.  시나리오 2. name 조회하기#2.2.2.  느린 이유 분석|2.2.2.  느린 이유 분석]]
	- [[#2.  본격 실험#2.3.  시나리오 3. name을 WHERE 절에 사용 🔐|2.3.  시나리오 3. name을 WHERE 절에 사용 🔐]]
	- [[#2.  본격 실험#2.4.  ⛔LIKE 쿼리가 안 좋은 이유|2.4.  ⛔LIKE 쿼리가 안 좋은 이유]]
- [[#성능 비교#3.  인덱스 도입|3.  인덱스 도입]]
	- [[#3.  인덱스 도입#3.1.  인덱스 만들기|3.1.  인덱스 만들기]]
	- [[#3.  인덱스 도입#3.2.  만약 인덱스 + Like를 쓴다면?|3.2.  만약 인덱스 + Like를 쓴다면?]]
...
# 성능 비교 
## 1.  성능 비교하기 전 세팅
> 인덱싱이 있고 없고에서의 성능을 비교하기전에 미리 세팅 
...
```
*Index Only Scan* 
- 일단 **Heap영역에 접근하지 않았다는 것을 보면 빠를 것이라고 예상**할 수 있다.
