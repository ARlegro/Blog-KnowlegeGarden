---
{"dg-publish":true,"permalink":"/DevStudy/DB/이전 프로젝트 - 최적화 해보기/","noteIcon":"","created":"2025-12-09T17:19:47.528+09:00","updated":"2025-12-13T09:26:31.584+09:00"}
---
...
(링크 : [Monew프로젝트](https://github.com/ARlegro/monew))
...
당시 프로젝트 마감과 자격증, 크래프톤 일정과 겹치면서 성능 최적화에는 충분한 시간을 들이지 못했고, **당시에는 RDB에 대한 이해도 부족으로 인해 단순한 조인 및 한방 쿼리 중심으로 코드를 구성된** 프로젝트였습니다. **이후** DB 관련 학습을 통해 이전 프로젝트들을 돌아보면서 **코드 전반에 심각한 성능 이슈가 있음을 인지**하게 되었습니다.
특히 다음 메서드에서 치명적인 성능 문제가 발생하였습니다 💢
```java
...
**여러 테이블을 조인해서 한방 쿼리로 만들었던 이 코드를 최적화해보려**고 합니다. 
...
1. *연관관계가 없는 JOIN이 일어났다* (`Subscription` - `ArticleInterest`)
	- *상황* : PK-FK로 직접 연결된 관계가 아니라 간접적으로 연결된 두 테이블을 서로 직접적인 관계인 것처럼 묶어서 JOIN한 것
	- 💢**Row Explosion이 발생**한다. (모든 조합을 다 만들어버리는...)
	- 실제로 EXPLAIN을 봐도 처음에는 약 400억 건 정도의 튜플이 예상된다고 나온다(뒤에서 Group by로 압축되지만)
		```sql
		  ->  Merge Join  (cost=70337.53..608317776.98 rows=40541991591 width=48)
		```
	- 즉, **폭발적인 튜플을 발생**시키고 이를 압축하기 위한 **불필요한 연산이 필요**해진 것
...
---
## 3.  쿼리 분리 및 최적화로 성능 개선 
> 결과 Preview : 50M~100M 데이터를 대상 : 20m+ 무한 로딩 ➡ 125ms로 감축

기존에는 테이블을 조인해서 1방 쿼리를 날렸지만, 이번 성능 최적화를 위해 만든 쿼리는 총 3방의 쿼리가 나갈 것입니다.

### 3.1.  쿼리 최적화 개선안 주요 핵심 
...
>[!TIP] 결국, 모든 핵심은 `Interest`를 중심으로 시작된다.  
...
#### 3.2.3.  쿼리 2. 관심사 정보 조회 - 1번에서 얻은 interestId 활용
```SQL 
...
---
## 4.  인덱스 최적화로 추가 성능 개선 시도

참고 : [[DevStudy/DB/Indexing/Key vs Non-Key column Index\|Key vs Non-Key column Index]]
...
- [[DevStudy/DB/Indexing/Covering Index VS Composite Index\|Covering Index VS Composite Index]]
...
---
### 4.3.  Index-Only-Scan임에도 생각보다 성능최적화가 안되는 이유 분석 
